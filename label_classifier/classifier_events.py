...
def classify_email(role, email):
    ...
    if 'events' in email['sender'].lower() or 'events' in email['subject'].lower():
        return True
    # if 'party' in subject or 'party' in from then label as events
    if 'party' in email['sender'].lower() or 'party' in email['subject'].lower():
        return True
    # if 'RSVP' in subject or 'RSVP' in from then label as events
    if 'rsvp' in email['sender'].lower() or 'RSVP' in email['subject'].lower():
        return True
    ...
    if 'conference' in email['sender'].lower() or 'conference' in email['subject'].lower():
        return True
    return False